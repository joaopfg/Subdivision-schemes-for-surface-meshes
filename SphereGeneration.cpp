...
class TriangleHashFunction
{
public:
  size_t operator()(const Triangle &t) const
  {
  	return (int)(((ll)t.a * (ll)t.b * (ll)t.c)%mod);
  }
};
...
public:
	...
	void subdivide()
	{
		...
		for(int i=0;i<n;i++){
			...
			V1(i,0) = V1(i,0)/sqrt(V1(i,0)*V1(i,0) + V1(i,1)*V1(i,1) + V1(i,2)*V1(i,2));
			V1(i,1) = V1(i,1)/sqrt(V1(i,0)*V1(i,0) + V1(i,1)*V1(i,1) + V1(i,2)*V1(i,2));
			V1(i,2) = V1(i,2)/sqrt(V1(i,0)*V1(i,0) + V1(i,1)*V1(i,1) + V1(i,2)*V1(i,2));
		}
		...
		cont = 0;
		unordered_map<Triangle, bool, TriangleHashFunction> visit;
		unordered_map<Triangle, bool, TriangleHashFunction>::iterator it;
		...
		for(int i=0;i<e;i++){
			...
			Triangle P2P1Pc = Triangle(P2, P1, Pc);
			Triangle P1P6Pc = Triangle(P1, P6, Pc);
			Triangle P4P3Pc = Triangle(P4, P3, Pc);
			Triangle P5P4Pc = Triangle(P5, P4, Pc);
			Triangle P3P2Pc = Triangle(P3, P2, Pc);
			Triangle P6P5Pc = Triangle(P6, P5, Pc);
...
private:
	...
	MatrixXd computeEdgePoint(int h)
	{
		...
		ans(0,0) = ans(0,0)/sqrt(ans(0,0)*ans(0,0) + ans(0,1)*ans(0,1) + ans(0,2)*ans(0,2));
		ans(0,1) = ans(0,1)/sqrt(ans(0,0)*ans(0,0) + ans(0,1)*ans(0,1) + ans(0,2)*ans(0,2));
		ans(0,2) = ans(0,2)/sqrt(ans(0,0)*ans(0,0) + ans(0,1)*ans(0,1) + ans(0,2)*ans(0,2));
